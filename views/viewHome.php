				...
				<div class="card h-100">
					<div class="single-post post-style-1" style="padding-bottom:0%">
						<div class="blog-image" style="position: relative;">
							...
							<h1 class="title" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);"><b><?= substr($post->getTitle(), 0, 10) ?></b></h1>
						</div>
						<div class="blog-info">
                            <h4 class="title" style="padding: 20px 10px 0px"><a href="post&id=<?= $post->getId() ?>"><b><?= substr($post->getChapo(), 0, 30). "..." ?></b></a></h4>
						</div>
						<div class="blog-info" style="padding: 20px 10px 0px">
							<h6 class="title" style="padding: 0px 10px 10px"><?= substr($post->getContent(), 0, 200). "..." ?></h6>
						</div>
						<div class="blog-info" style="padding: 10px 10px 0px">
							<?php if ($post->getDateUpdated()): ?>
								<h6 class="title" style="padding: 0px 10px 10px;text-align: left;">Dernière modification le : <?= $post->getDateUpdated()->format('d/m/Y') ?></h6>
							<?php else: ?>
								<h6 class="title" style="padding: 0px 10px 10px;text-align: left;">Publié le : <?= $post->getDateCreated()->format('d/m/Y') ?></h6>
							<?php endif; ?>
						</div>
					</div>
				</div>
